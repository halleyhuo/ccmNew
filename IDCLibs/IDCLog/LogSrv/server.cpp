...
#define CREATE_FILE_ATTR "a+t"
#endif
	static int file_cnt = 0;//the counter of record file	

	inline int napms( int uiDelay){
	...
	static int CreateFile(char _filePath[64])
	{
		stream = NULL;
		...
		while(true){
			...
			if (iRetval < 0)
			{
				...
				*/
                ...
                //printf("TobeLog %s\n",bufToBeLog);
                const int emptyRoom = sizeof(bufToBeLog)-strnlen(bufToBeLog,sizeof(bufToBeLog));
				...
				const uchar logSeverity = dataBuffer.logSeverity;
