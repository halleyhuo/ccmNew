...
int sockExt[MAX_PORT_SUPPORT] = {-1,-1}; 
...
static int SendtoClient( tXtpMessage *InTel,int _DeviceNum=0 )
{
	int retVal = -1;
	if (InTel!=NULL&&InTel->msgHeader.length>=sizeof(tXtpObjHeader))
	{
		const uchar TargetNodeID = InTel->msgHeader.target;
		...
		else if(InTel->msgHeader.mode == XTP_MODE_BROADCAST)
		{
            ...
            RecordXTP( "XTP Msg %xH(%d) mode %d %xH %s", 
		            InTel->msgHeader.telID,InTel->msgHeader.length,InTel->msgHeader.mode,InTel->msgHeader.sender,targetBuf);
		}
...
{      
	...
	while(INRUNNING) {	
		...
		FD_ZERO(&readfds);
		int max_sd = 0;
		for(int i=0; i<MAX_PORT_SUPPORT; i++){
		...
		if ((activity < 0) && (errno!=EINTR)) 
		{
			...
			for (int i = 0; i < MAX_PORT_SUPPORT; i++) 
			{
				...
				{
					/* Read a message back */
					int bytes_read = read( sockExt[i], buf, sizeof(buf) );
					if(bytes_read<0){
...
void * app_handler (void *arg)
{
	...
	if(msgq_create(100, sizeof(tXtpMessage)))
	{
		...
		while(INRUNNING)
		{  
            const int rc = msgq_recv(&Command);
			if ( rc>0 )
			{
				if((Command.bCommand != XtpSysStat)  &&(Command.bCommand != XtpSysReceive)
					&&(XtpSysPulse!=Command.bCommand)){
						IDC::Log(IDC::LOG_INFO,LOG_TAG,"Rcv (%d) from App Cmd:%d from PID:%d", rc,
							Command.bCommand,Command.pid);
				}
				...
				case XtpSysInit:
					{                    
						if(Command.msgHeader.telID==SCONNECT)
						{
							...
							{
								IDC::Log(IDC::LOG_INFO,LOG_TAG,"Got SCONNECT(%d) from Client 0x%x Process %d ", 
									Command.msgHeader.length,Command.chid,Command.pid);
								...
								else
								{
                                    ...
                                    RecordXTP("Node %xH Connect to the XTP", 
		                                Command.chid);
								}                      
							...
							{
								IDC::Log(IDC::LOG_ERR,LOG_TAG,"Length Fail objCnt %d act Length %d",
									objCnt,Command.msgHeader.length);
							}

						}
						else
						{//Command.msgHeader.telID
							IDC::Log(IDC::LOG_WARNING,LOG_TAG,"Illegal TelID 0x%x (%d) for SysInit", 
								Command.msgHeader.telID,Command.msgHeader.length);
						}
					}
					break;
				case XtpSysExit:
                {
                    ...
                    if(clientMsgQ!=NULL)
                    {
            			...
            			answer.bState = XtpOk;	
                        conn_channel_send_l(clientMsgQ,
        				    &answer,
...
{ 
	...
	int ret=pthread_attr_setstacksize(&attr, STACK_SIZE_DEFAULT);
	if (ret != 0)
