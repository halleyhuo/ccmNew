...
#define ADDR_SRV "192.168.8.13"
	const unsigned short PORT_SRV = 2288;

#define ADDR_SELF "192.168.8.30"
	const unsigned short PORT_SELF = 2288;

#define ADDR_SIMU "192.168.8.15"
const unsigned short PORT_SIMU = 2288;

static int sockudpsend[ExtLinkSize] = {0,0};
...
void RecordXTP(const char* const fmt, ...)
{
	if(sockudpsend[ExtLinkManager]!=0)
	{
		...
		int iSendSize = sendto(sockudpsend[ExtLinkManager], &buf, buf.FrameLen, 0,  
