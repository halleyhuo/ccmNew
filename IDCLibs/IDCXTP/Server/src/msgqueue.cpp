...
void msg_q_init(int count, int size )
{
	mq_info.mq        = -1;
	mq_info.ErrId     = EOK;
	//mq_info.ErrMsg    = "";
...
bool msgq_create(int count, int size)
{
...
static int conn_channel_get(int pid, int chid)
{
	int conn = -1;
	int i = 0;
...
CIDCMsgQueue* conn_channel_get(const uchar NodeID)
{
...
static CIDCMsgQueue* conn_channel_attach(int pid, int chid)
{
	int iRetry = 5;
...
int conn_channel_send_l( CIDCMsgQueue* msgQSrv2Client, void *buf, int len)
{
	int iRetval = -1;
	int iRetry = 10;
...
//int conn_channel_send( CIDCMsgQueue* msgQ, void *buf, const int len )
//{
//	return conn_channel_send_l(msgQ,buf,len);
//}

int conn_channel_send( void* buf, const int len,const int pid, const int chid )
{
	int conn_idx = conn_channel_get(pid, chid);
	int rc = -1;
...
int conn_channel_send(void* buf, const int len)
{
	int rc = -1;     
	if(mq_info.mq!=-1 && buf!=NULL && len<=sizeof(tXtpMessage))
	{       
		int iRetry = 5;//Try 5 time
		do
		{                
...
bool conn_channel_add(int pid, int chid, int bus/*= 0*/)
{
	bool rc = false;
...
CIDCMsgQueue* conn_channel_del(int pid, int chid)
{	
...
int conn_channel_recv(void* buf)
{
	int iRetval = -1;
	int iRetry  = 10;    
...
int conn_channel_reset_alarm(const int pid, const int chid)
{
	const int conn_idx = conn_channel_get(pid, chid);
	if(conn_idx != -1 )
	{
		conn_list[conn_idx].alarm = 0;
	}		
...
int conn_channel_filter(int pid, int chid, unsigned short Id, unsigned char add)
{
	int conn_idx;
...
int conn_channel_answer (int pid, int chid, unsigned char command, unsigned char state)
{
	int conn_idx;
	bool rc = -1;
	//find connection
...
int conn_channel_filter_add(int index, int id)
{
	int i = 0;
...
int conn_channel_filter_del(int index, int id)
{
	int i = 0;
...
CIDCMsgQueue* conn_channel_filter_get(int id, int& pos, unsigned char& target )
{
	...
	{        
		if( conn_list[pos].bus_config )
		{
            ...
            int j = 0;
			for(int i=0; i<MAX_ID_FILTER_EACH_CHN; i++)
			{
...
int conn_channel_bus_get(int pid, int chid)
{
	int conn_idx;
