...
BRODCAST_LIST* GetBroadList(const uchar NodeID)
{
	BRODCAST_LIST* rc =NULL;
	for(int i=0; i<sizeof(broadcastList)/sizeof(broadcastList[0]); i++)
	{
...
CXtpClient::CXtpClient(const uchar id)
...
int CXtpClient::CreateChannelSrv2Clt()
{
	int rc = -1;
	if ( m_own_msgQ ==NULL )
	{
...
bool CXtpClient::XtpSend (tXtpObjHeader *msg, uchar command /* =XtpSysSend*/,void* extralPara/* = NULL*/)
{
	...
	IDC::Log(IDC::LOG_INFO,LOG_TAG,"Send Out:cmd %d send fd %d ",
		command,m_ServerQueue);
...
void* CXtpClient::ReceiveThread (void*pArg)
{
	...
	if(pArg!=NULL)
	{
		...
		{
			IDC::Log(IDC::LOG_DEBUG,LOG_TAG,"Prepare call recv,pid %d NodeID 0x%x",
				getpid(),xtpClient->m_XTP_Node);
			xtpClient->m_semaPReadyforRcv=true;
			int recvmsgcnt = xtpClient->m_own_msgQ->recv(pAnswer, sizeof(tXtpMessage));   		 
			...
			{
				IDC::Log(IDC::LOG_ERR,LOG_TAG,"###error, call Rcv self MsgQ Fail,pid %d NodeID 0x%x",
					getpid(),xtpClient->m_XTP_Node);
			}
			else
			{
				IDC::Log(IDC::LOG_INFO,LOG_TAG,"recv result as 0,pid %d NodeID 0x%x",
					getpid(),xtpClient->m_XTP_Node);
